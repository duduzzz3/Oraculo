...
def preparar_dataframe_obras(df: pd.DataFrame, site: str | None = None) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        registros.append({
            ...
            "Link da imagem da obra": limpar_texto(row.get("Link da imagem da obra")),
        })
...
def sanear_acervo() -> int:
    ...
    with conectar() as conn:
        ...
        for row in rows:
            ...
            if any([novo_nome != row["nome_obra"], novo_autor != row["autor"], nova_tecnica != row["tecnica"], tecnica_original != row["tecnica_original"], novo_ano != row["ano_obra"], nova_dim != row["dimensoes"]]):
                conn.execute(
...
def aba_coleta(df: pd.DataFrame) -> None:
    ...
    st.caption("Use 0 para tentar coletar o máximo disponível. Também é possível executar os scrapers específicos de Cícero Dias, Milan Dusek e Rubem Grilo.")
